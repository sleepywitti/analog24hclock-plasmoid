...
function morningTime(
  ...
  var timeMins =
    phenomenon(julian, lat, -long, zenithDistance, 1) +
    timeZone_raw_offset / 60000.0;
  if (dst) timeMins += 60.0;
...
function phenomenon(julian, latitude, longitude, zenithDistance, hour_sign) {
  ...
  var newt = julianDayToJulianCenturies(
    julianCenturiesToJulianDay(t) + timeUTC / 1440,
  );

  timeUTC = phenomenonHelper(
    newt,
    latitude,
    longitude,
    zenithDistance,
...
function hourAngle(lat, solarDec, zenithDistance) {
  ...
  return acos(
    cos(rad(zenithDistance)) / (cos(phi) * cos(delta)) - tan(phi) * tan(delta),
  );
}
...
function obliquityCorrection(t) {
  return meanObliquityOfEcliptic(t) + 0.00256 * cos(rad(125.04 - 1934.136 * t));
}
...
function sunsApparentLongitude(t) {
  return (
    sunsTrueLongitude(t) - 0.00569 - 0.00478 * sin(rad(125.04 - 1934.136 * t))
  );
}
...
function equationOfCentreForSun(t) {
  ...
  return (
    ...
    sin(2 * rad(m)) * (0.019993 - 0.000101 * t) +
    sin(3 * rad(m)) * 0.000289
  );
}
